...
limitClause
    : LIMIT limit=limitOperand (OFFSET offset=limitOperand)?
    | OFFSET offset=limitOperand (LIMIT limit=limitOperand)?
    ;
...
classExprOperand
    : IDENTIFIER alias=IDENTIFIER? pathPredicate?
    ;
...
identifiedPath
    : IDENTIFIER (SYM_LEFT_BRACKET nodePredicate SYM_RIGHT_BRACKET)? (SYM_SLASH objectPath)?
    ;

pathPredicate
    : SYM_LEFT_BRACKET ALL_VERSIONS (SYM_COMMA nodePredicate)? SYM_RIGHT_BRACKET
    | SYM_LEFT_BRACKET LATEST_VERSION (SYM_COMMA nodePredicate)? SYM_RIGHT_BRACKET
    | SYM_LEFT_BRACKET nodePredicate SYM_RIGHT_BRACKET
    ;

nodePredicate
    : (leftIdCode=ID_CODE | leftAtCode=AT_CODE | leftParamter=PARAMETER | ARCHETYPE_HRID) (SYM_COMMA (rightAtCode=AT_CODE | rightIdCode=ID_CODE | rightParameter=PARAMETER | STRING | TERM_CODE))?
...
objectPath
    ...
    ;
pathPart
    : IDENTIFIER (SYM_LEFT_BRACKET nodePredicate SYM_RIGHT_BRACKET)?
    ;
...
functionCall
    : terminologyFunction
    | name=(STRING_FUNCTION_ID | NUMERIC_FUNCTION_ID | DATE_TIME_FUNCTION_ID) SYM_LEFT_PAREN (terminal (SYM_COMMA terminal)*)? SYM_RIGHT_PAREN
    ;
...
fragment ISO8601_TIME
    : HOUR ':' MINUTE ':' SECOND ('.' MILISECOND)? TIMEZONE?
    ;
fragment ISO8601_DATE_TIME
    : YEAR '-' MONTH '-' DAY ('T' HOUR ':' MINUTE ':' SECOND ('.' MILISECOND)? TIMEZONE?)?
    ;
fragment MILISECOND: [0-9][0-9][0-9];
