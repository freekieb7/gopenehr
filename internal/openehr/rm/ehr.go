...
func (e *EHR) Validate(path string) util.ValidateError {
	...
	if e.EHRStatus.Type != EHR_STATUS_TYPE && e.EHRStatus.Type != VERSIONED_EHR_STATUS_TYPE {
		validateErr.Errs = append(validateErr.Errs, util.ValidationError{
			Model:          EHR_TYPE,
			Path:           attrPath,
	...
	if e.EHRAccess.Type != EHR_ACCESS_TYPE && e.EHRAccess.Type != VERSIONED_EHR_ACCESS_TYPE {
		validateErr.Errs = append(validateErr.Errs, util.ValidationError{
			Model:          EHR_TYPE,
			Path:           attrPath,
	...
	if e.Compositions.E {
		for i := range e.Compositions.V {
			...
			if e.Compositions.V[i].Type != COMPOSITION_TYPE {
				validateErr.Errs = append(validateErr.Errs, util.ValidationError{
					Model:          EHR_TYPE,
					Path:           attrPath,
					Message:        fmt.Sprintf("invalid composition type: %s", e.Compositions.V[i].Type),
					Recommendation: fmt.Sprintf("Ensure compositions[%d] _type field is set to '%s'", i, COMPOSITION_TYPE),
				})
			}
