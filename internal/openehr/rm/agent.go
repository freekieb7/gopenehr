...
func (a *AGENT) Validate(path string) util.ValidateError {
	...
	// Validate _type
	attrPath = path + "._type"
	if a.Type_.E && a.Type_.V != AGENT_TYPE {
		validateErr.Errs = append(validateErr.Errs, util.ValidationError{
			Model:          AGENT_TYPE,
			Path:           attrPath,
			Message:        "invalid " + AGENT_TYPE + " _type field: " + a.Type_.V,
			Recommendation: "Ensure _type field is set to '" + AGENT_TYPE + "'",
		})
	}
