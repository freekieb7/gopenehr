...
func PocUnmarshal(value reflect.Value, source any) error {
	switch value.Kind() {
	case reflect.Bool:
		{
			b, ok := source.(bool)
	...
	case reflect.Struct:
		{
			if u, ok := value.Interface().(Marshaller); ok {
			...
			for i := range rt.NumField() {
				...
				if tag == "" {
					return errors.New("missing openehr tag")
				}

				fv := reflect.New(field.Type.Elem())

				PocUnmarshal(fv, v[tag])
			}
		}
	default:
		{
...
func Unmarshal(value any, source []byte) error {
	scanner := Scanner{
		State:        STATE_VALUE,
		Input:        source,
		Cursor:       0,
...
func unmarshal(value reflect.Value, scanner *Scanner) error {
	switch value.Kind() {
	case reflect.Bool:
		{
			token, err := scanner.Next()
