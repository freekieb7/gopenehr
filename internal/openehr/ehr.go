...
func (e *EHR) Validate(path string) []util.ValidationError {
	...
	if e.EHRStatus.Type != EHR_STATUS_MODEL_NAME {
		errs = append(errs, util.ValidationError{
			Model:          EHR_MODEL_NAME,
			Path:           attrPath,
			Message:        fmt.Sprintf("invalid EHR status type: %s", e.EHRStatus.Type),
			Recommendation: fmt.Sprintf("Ensure ehr_status _type field is set to '%s'", EHR_STATUS_MODEL_NAME),
		})
	...
	if e.EHRAccess.Type != EHR_ACCESS_MODEL_NAME {
		errs = append(errs, util.ValidationError{
			Model:          EHR_MODEL_NAME,
			Path:           attrPath,
			Message:        fmt.Sprintf("invalid EHR access type: %s", e.EHRAccess.Type),
			Recommendation: fmt.Sprintf("Ensure ehr_access _type field is set to '%s'", EHR_ACCESS_MODEL_NAME),
		})
	}
